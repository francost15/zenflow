...
void registerAppRedesignTests() {
  ...
  testWidgets('ProfileScreen shows achievements after moving them out of streaks', (
    tester,
  ) async {
  ...
  testWidgets('CoursesScreen renders derived progress and upcoming class data', (
    tester,
  ) async {
  ...
  testWidgets('TaskEditorSheet preloads existing task data and exposes delete', (
    tester,
  ) async {
    final now = DateTime.now();
    final course = Course(
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: MultiBlocProvider(
          providers: [
            BlocProvider(create: (_) => TaskBloc(FakeTaskRepository(tasks: [task]))),
            BlocProvider(
              create: (_) => CourseBloc(
                FakeCourseRepository(courses: [course]),
