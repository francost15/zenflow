...
void main() {
  group('Missing root planning files are rejected', () {
    test('ROADMAP.md missing required columns is rejected', () async {
      ...
      io.File(
        '${tmpDir.path}/STATE.md',
      ).writeAsStringSync('## Current Phase\n## Score Snapshot');
      io.File(
        '${tmpDir.path}/SCORECARD.md',
      ).writeAsStringSync('## Task Scores\n| Task ID | Plan ID | Estimated |');
      io.File(
        '${tmpDir.path}/AGENT_POLICY.md',
      ).writeAsStringSync('## Score Bands\n## Skill Rules');
  ...
  group('Verification and dependency requirements', () {
    test('blocks phase closure when required verification file missing', () {
      ...
      expect(
        result.blockers.any(
          (blocker) => blocker.contains('verification artifact'),
        ),
        true,
      );
    });
    ...
    test('blocks phase advancement when dependency phase not closed', () {
      ...
      expect(
        result.blockers.any(
          (blocker) => blocker.contains('Open dependency phases'),
        ),
        true,
      );
    });
